...
const MapView: React.FC<MapViewProps> = ({ 
  ...
  useEffect(() => {
    ...
    try {
      ...
      map.current = new mapboxgl.Map({
        ...
        center: [-46.6333, -23.5505], // São Paulo, Brasil por padrão
        zoom: 12
      });
  ...
  useEffect(() => {
    ...
    buses.forEach(bus => {
      ...
      el.innerHTML = `
        <div class="${selectedBusId === bus.id ? 
          'bg-busapp-secondary animate-pulse-slow' : 
          'bg-busapp-primary'} p-2 rounded-full cursor-pointer shadow-lg">
